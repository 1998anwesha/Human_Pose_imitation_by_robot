...
#source: https://github.com/nicknochnack/MediaPipePoseEstimation
def pose_detect():
    ...
    ## Setup mediapipe instance
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            while cap.isOpened():
                ...
                # cv2.resizeWindow('Mediapipe Feed', 300, int(h * (300 / w)))        
...
def handle_connect():
    ...
    # Start pose detection thread when the first client connects
    # pose_detect()
    pose_thread = Thread(target=pose_detect)
    pose_thread.start()
